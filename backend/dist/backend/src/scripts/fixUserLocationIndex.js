...
const fixUserLocationIndex = async () => {
    try {
        ...
        const geoIndexes = indexes.filter(index => index.key.location === '2dsphere' ||
        ...
        const gigGeoIndexes = gigIndexes.filter(index => JSON.stringify(index.key).includes('2dsphere'));
...
//# sourceMappingURL=fixUserLocationIndex.js.map