...
export const getDashboardStats = async (req, res) => {
    try {
        ...
        const stats = {
            ...
            activeGigs: userPostedGigs.filter(gig => ['posted', 'active', 'assigned', 'in_progress'].includes(gig.status)).length,
            completedGigs: userPostedGigs.filter(gig => gig.status === 'completed').length,
            draftGigs: userPostedGigs.filter(gig => gig.status === 'draft').length,
            ...
            totalApplicationsReceived: userPostedGigs.reduce((sum, gig) => sum + (gig.applicationsCount || 0), 0),
        };
        // Calculate user's application statistics
        let userApplications = [];
        userApplicationsData.forEach(gig => {
            const userApps = gig.applications.filter((app) => app.applicantId.toString() === userId.toString());
            userApplications.push(...userApps.map((app) => ({
                ...app,
                gigPayment: gig.payment
            })));
        });
        stats.totalApplications = userApplications.length;
        stats.pendingApplications = userApplications.filter(app => app.status === 'pending').length;
        stats.acceptedApplications = userApplications.filter(app => app.status === 'accepted').length;
        stats.rejectedApplications = userApplications.filter(app => app.status === 'rejected').length;
        // Calculate earnings from completed gigs where user was assigned
        const completedAsWorker = userApplicationsData.filter(gig => {
            const userApp = gig.applications.find((app) => app.applicantId.toString() === userId.toString() &&
                app.status === 'accepted');
